...
class ImportDirectXXFile:
    ...
    def create_obj_from_node(self, matrix: mathutils.Matrix, node: XModelNode, filepath: str):
        if matrix is None:
            matrix = mathutils.Matrix.Identity(4)

        first_obj = None
        for child in node.children:
            child = self.create_obj_from_node(matrix @ child.transform_matrix, child, filepath)
        ...
        model_name = (node.node_name if node.node_name is not None and len(node.node_name) != 0 else os.path.splitext(bpy.path.basename(filepath))[0]) + str(self.object_index)
        self.object_index += 1
        ...
        return created_here if created_here is not None else first_obj
    ...
    def execute(self, filepath: str):
        ...
        obj = self.create_obj_from_node(mathutils.Matrix.Identity(4), root_node, filepath)
